...
<form action="<?php echo JRoute::_('index.php?option=com_phocadownload&view=phocadownloadlocations'); ?>" method="post" name="adminForm" id="adminForm">
        ...
        <table class="adminlist">
            <thead>
                <tr>
                    ...
                    <th width="5%" nowrap="nowrap"><?php echo JText::_('COM_PHOCADOWNLOAD_UNITS'); ?>
                    </th>
                    ...
                    <th width="8%">
                        ...
                        if ($canOrder && $saveOrder) {
                            echo JHtml::_('grid.order', $this->items, 'filesave.png', 'phocadownloadlocations.saveorder');
                        }
                        ?>
                    </th>
            ...
            <tbody>
                ...
                if (is_array($this->items)) {
                    ...
                    foreach ($this->items as $i => $item) {
                        ...
                        $unitsArray = $registry->toArray();
                        
                        $units = PhocaDownloadUnits::getListItem($unitsArray);
                        ...
                        $linkEdit = JRoute::_('index.php?option=com_phocadownload&task=phocadownloadlocation.edit&id=' . (int) $item->id);
                        ...
                        echo '<td>';
                        if ($item->checked_out) {
                            echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'phocadownloadlocations.', $canCheckin);
                        }
                        ...
                        echo '<td class="center">' . JHtml::_('jgrid.published', $item->published, $i, 'phocadownloadlocations.', $canChange) . '</td>';
                        
//                       ########1##### 
//                        echo '<td class="center">'
//                             .   '<select class="inputbox" multiple="multiple" size="3" style="width: 250px;">'
//				.	JHTML::_('select.options',  $units )
//				.'</select>'
//                             .'</td>';
//                       ########2##### 
//                        $element[] = '<td class="center">';
//                        $element[] = '<textarea rows="2" cols="8" readonly="true" style="resize: none;">';
//                        foreach ($units as $unit){
//                            $exp = explode('-', $unit->get('text'));
//                            $element[] = trim($exp[0]).';';
//                        }
//                        
//                        $element[] = '</textarea>';
...
//                        echo implode("\n", $element);
                        ...
                        foreach ($units as $unit){
                            $icon = ($unit->published == '1')?'[ <img src="'.  JURI::base().'templates/bluestork/images/admin/tick.png" /> Publicado ]':'[ <img src="'.  JURI::base().'templates/bluestork/images/admin/publish_x.png" /> Despublicado ]';
                            $toolTips[] = $unit->text.' - '.$icon;
                        }
                        $element[] = JHtml::tooltip(implode("<br />", $toolTips), 'Unidade(s) de '.$this->escape($item->title), '', '<img src="templates/bluestork/images/admin/icon-16-eye.png" />');
